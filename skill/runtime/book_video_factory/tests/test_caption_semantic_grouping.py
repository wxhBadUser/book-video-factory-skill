...
@pytest.mark.parametrize(
    ("action_state", "risk_reason"),
    [
        ("福贵面对死亡的高潮。", "climax_or_death"),
        ("福贵的英雄特写定格。", "hero_shot"),
        ("福贵翻越悬崖。", "high_risk_action"),
    ],
)
def test_new_high_risk_scene_moment_has_an_explicit_split_reason(
    action_state: str, risk_reason: str
) -> None:
    first = _contract("caption-0001", "福贵坐在田埂上。")
    second = _contract("caption-0002", "下一句字幕。", action_state=action_state)
...
def test_mutually_exclusive_actions_require_an_explicit_split() -> None:
    first = _contract("caption-0001", "福贵站起。", action_state="福贵站起。")
    second = _contract("caption-0002", "福贵坐下。", action_state="福贵坐下。")
    ...
    assert groups[1].split_from_previous == {
        "required": True,
        "reasons": ["mutually_exclusive_action"],
    }


def test_duration_cap_splits_same_scene_without_honoring_a_merge_hint() -> None:
    contracts = {
